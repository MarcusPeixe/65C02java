...
class Token {
    ...
    final static Set<String> opcodes = new HashSet<>(Set.of(
    ...
    final static Set<String> directives = new HashSet<>(Set.of(
...
//        System.out.println(s);
    ...
    public boolean isEndOfLine() {
        return isOpcode() || isLabel() || isDirective();
    }

    @NotNull
    public static String printLineError(String s, @NotNull ParseException e) {
        ...
        try {
            ...
            err.append(" ".repeat(column));
            err.append("^".repeat(length));
            err.append("\n");
        }
        catch (Exception ex) {
            err.append(lineStr);
...
//                System.out.printf("%s: abs %s\n", t, e);
    ...
    public ASTexp parseExp6() throws ParseException { // PARENTHESES
        if (peek().is('(')) {
            consume();
            ASTexp e = parseExp();
            if (!peek().is(')')) {
    ...
    public ASTexp parseExp7() throws ParseException { // NEG LO HI & NUM
        if (peek().isIn("-<>")) {
            Token op = peek();
            consume();
            ASTexp e = parseExp7();
...
//        clearMemory();
    ...
    private static Map<String, Integer> initMap() {
        final List<String> opcodeList = new ArrayList<>(List.of(
